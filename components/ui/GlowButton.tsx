...
export function GlowButton({ children, variant = 'cyan', effect = 'none', className, ...props }: GlowButtonProps) {
  ...
  const styles =
    variant === 'orange'
      ? 'bg-orange-500 text-white shadow-[0_0_30px_rgba(249,115,22,.45)] hover:bg-orange-400'
      : variant === 'outline'
        ? 'border border-cyan-400/60 text-cyan-200 hover:bg-cyan-500/10'
        : 'bg-cyan-500 text-slate-950 shadow-[0_0_30px_rgba(0,180,240,.45)] hover:bg-cyan-400';

  return (
    <motion.button
      animate={reduce || effect === 'none' ? undefined : { scale: [1, 1.015, 1] }}
    ...
    >
      <span className='relative z-10'>
        {children}
        {variant !== 'outline' && <ArrowRight className='ml-2 inline h-4 w-4' />}
      </span>
      {effect === 'shimmer' && !reduce && (
